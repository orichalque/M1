...
public class CSVToTTL {
	...
	public void read() throws IOException{
      ...
      if (csv.isEmpty()) {
        ...
        writer.println("@prefix ns0: <http://erdf.opendatasoft.com/projetwebsem/parc-raccorde-par-tranches-csv#> .");
        writer.println("@prefix dbpedia: <http://fr.dbpedia.org/page#> .");
        writer.println("@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .");
        writer.println("@prefix erdf: <http://erdf.org#> .");
        writer.println("@prefix semanco: <http://semanco-tools.eu/ontology-releases/eu/semanco/ontology/SEMANCO/SEMANCO.owl#> .");
        ...
        for (int k = 1; k < csv.size(); ++k) {
          ...
          for(String s : tab){
            ...
            if (inner != 1){
            	...
            	writer.println('\t' + domain + ":pId\t\"" + id + "\"^^xsd:String;");
            	writer.println('\t' + domain + ":pMin\t\"" + min + "\"^^xsd:Double;");
            	writer.println('\t' + domain + ":pMax\t\"" + max + "\"^^xsd:Double;");
            	writer.print('\t' + domain + ":pUnit\t\"" + unit + "\"^^xsd:String");          	
            }
