...
public class Timer {
     ...
     * @param hour choix d'heure entre 0 et 23
     * @param min choix de minutes entre 0 et 59
     * @param calendar stub du calendar
     * @throws TimerException un des parametres est hors-limite
     */
    public Timer(int ring, int hour, int min, Calendar stub) throws TimerException {
    	this.stubCalendar = stub;
        if (hour < 0) {
            throw new TimerException("bad hour: inf value");
        } else if (hour > 23) {
            throw new TimerException("bad hour: sup value");
    ...
    protected void setActive(boolean active) {
        this.active = active;
        if (this.active) { 
            if (this.hour == stubCalendar.get(Calendar.HOUR)) {
                if (this.min == stubCalendar.get(Calendar.MINUTE)) {
                    ringing = true;
                }
            }
        }
